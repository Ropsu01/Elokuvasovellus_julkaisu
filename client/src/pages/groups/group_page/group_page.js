  ...
  return (
    <div className="message">
      <div className="message-header">
        <img src={`/avatars/${message.avatar}`} alt={`${message.username}'s avatar`} className="message-avatar" />
        <strong 
...
function GroupPage() {
  ...
  const fetchJoinRequests = useCallback(async () => {
    try {
      const response = await axios.get(`/groups/${groupId}/requests`, {
        headers: {
  ...
  const fetchUserNews = useCallback(async () => {
    try {
      const response = await axios.get(`/groups/${groupId}/news`, {
        headers: {
          Authorization: `Bearer ${jwtToken.value}`,
        },
      });
  ...
  useEffect(() => {
    const fetchGroupDetails = async () => {
      try {
        const response = await axios.get(`/groups/${groupId}/details`, {
          headers: {
            Authorization: `Bearer ${jwtToken.value}`,
          },
        });
  ...
  const sendJoinRequest = async () => {
    try {
      await axios.post(`/groups/${groupId}/request-join`, {}, {
  ...
  const handleJoinRequest = async (requestId, accept) => {
    try {
      await axios.put(`/groups/${groupId}/requests/${requestId}`, { accept }, {
        headers: { Authorization: `Bearer ${jwtToken.value}` }
      });
  ...
  const handleRemoveMember = async (username) => {
    try {
      ...
      if (confirmRemove) {
        await axios.delete(`/groups/${groupId}/members/${username}`, {
          headers: { Authorization: `Bearer ${jwtToken.value}` },
        });
        ...
        if (isOwner) {
          ...
          if (confirmNewOwner) {
            ...
            if (newOwner && newOwner !== groupData.creator_username) {
              try {
                  await axios.put(`/groups/${groupId}/assign-owner/${newOwner}`, {}, {
                  headers: { Authorization: `Bearer ${jwtToken.value}` },
                });
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`/groups/${groupId}/message`, {
        message: newMessage,
      }, {
        headers: {
          Authorization: `Bearer ${jwtToken.value}`,
  ...
  const handleDeleteMessage = async (messageId) => {
    try {
        await axios.delete(`/groups/${groupId}/messages/${messageId}`, {
            headers: { Authorization: `Bearer ${jwtToken.value}` }
        });
            ...
            <div className="members-list">
              {groupMembers.map((member, index) => (
                <div className="member-item" key={index}>
                  <img src={`/avatars/${member.avatar}`} alt={`${member.username}'s avatar`} className="member-avatar" />
