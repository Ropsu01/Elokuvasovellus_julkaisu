...
function Profile() {
  ...
  useEffect(() => {
    if (userData.value) {
      ...
      if (userData.value && userData.value.avatar) {
        setUserAvatar(`/avatars/${userData.value.avatar}`);
      } else {
        setUserAvatar('/avatars/npc.png');
      }
  ...
  useEffect(() => {
    const fetchUserReviews = async () => {
      try {
        const response = await axios.get(`/review/user/${username}`);
        const userReviewsData = response.data;

        const movieIds = [...new Set(userReviewsData.map((review) => review.movie_id))];

        const movieResponses = await Promise.all(movieIds.map((id) => axios.get(`/movies/${id}`)));
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`/posts/`, { content: newPostContent }, {
        headers: { Authorization: `Bearer ${jwtToken.value}` }
      });
